...
date: 2025-06-07
---
二维数点问题的一般化形式已在[求解二维数点问题-离线](count-points-in-2-dimension-offline#一般化形式)给出，这里就不多赘述。
...
普通的值域线段树能够维护一个一维下的信息。宏观的看，可持久化线段树从修改时间的角度上维护了一连串值域线段树的前缀和，把修改时间看作另一维度，也就是说，**可持久化线段树天生可以维护二维信息**，所以我们尝试把它拿去解决二维数点问题。
...
3. 将第一步排好序的$P_i(x_i,y_i)$逐个修改$Tree$，修改方式是$node[y_i].val + 1$，在可持久化线段树的策略下，这会给整个线段树添加一条新链，这个新链则代表本次修改。然后我们记录下这次修改的$x_i$和所对应的$Tree$的根节点到一个`map`映射中。

### 查询

1. 可以通过"二分"$map$找到$a$对应的$lroot = \max_{x_i < a}{x_i}$ 与$b$对应的 $rroot = \min_{x_i \ge b}{x_i}$ ，这样从前缀和的角度，这两个根节点所代表的值域线段树的差$rroot-lroot$则能表示所有$P_i(x_i,y_i), a \le x_i \le b$。
...
下面是采用可持久化线段树在线解决该问题的代码，其效率比之前的更好。
